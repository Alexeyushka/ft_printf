...
struct	p print_flags_zero_d(const char *format, va_list list, int res, struct p parsed)
{
	int number;
	int number2;
...
struct	p parse_flag_zero_d(const char *format, va_list list, int i, struct p parsed)
{
	int res;
	int k;
	int count;
	...
	else
	{
		...
		parsed = print_flags_zero_d(format, list, res, parsed);
	}
	return (parsed);
}